...
class CameraDiscoveryApp:
    ...
    def create_camera_tab(self):
        ...
        self.scan_btn = ttk.Button(
            top_frame, text="Поиск GigE камер",
            command=self.scan_cameras, width=20
        )
        self.scan_btn.pack(side=tk.LEFT, padx=5)

        self.connect_btn = ttk.Button(
            top_frame, text="Подключиться",
            command=self.connect_camera, width=20, state=tk.DISABLED
        )
        self.connect_btn.pack(side=tk.LEFT, padx=5)

        self.show_video_btn = ttk.Button(
            top_frame, text="Показать видео",
            command=self.show_video, width=20, state=tk.DISABLED
        )
        ...
        self.status_bar = ttk.Label(
            self.camera_tab,
            text="Готов к работе. Нажмите 'Поиск GigE камер'",
            relief=tk.SUNKEN,
        )
    ...
    def create_video_tab(self):
        ...
        self.snapshot_btn = ttk.Button(
            controls_frame, text="Снимок экрана",
        ...
        self.video_status = ttk.Label(
            self.video_tab, text="Статус: Ожидание действия",
    ...
    def create_log_tab(self):
        ...
        ttk.Button(log_controls, text="Сохранить логи в файл",
        ...
        self.log_info("Нажмите 'Поиск GigE камер' для сканирования.")
    ...
    def create_context_menu(self):
        ...
        self.context_menu.add_command(label="Копировать IP адрес",
                                      command=self.copy_ip)
        self.context_menu.add_command(label="Копировать серийный номер",
                                      command=self.copy_serial)
        self.context_menu.add_separator()
        self.context_menu.add_command(label="Обновить список устройств",
            ...
            if values and len(values) >= 4:
                ...
                if ip and ip not in ('N/A', 'Unknown'):
                    ...
                    self.status_bar.config(
                        text=f"Скопировано в буфер обмена IP: {ip}"
                    )
        ...
        if selection:
            ...
            if values and len(values) >= 3:
                ...
                if serial and serial not in ('N/A', 'Unknown'):
                    ...
                    self.status_bar.config(
                        text=f"Скопировано в буфер обмена SN: {serial}"
                    )

    def scan_cameras(self):
        self.log_info("Сканирование сети на наличие Lucid Triton GigE...")
    ...
    def _scan_cameras_thread(self):
        try:
            ...
            self.log_success(
                f"Поиск завершён. Найдено камер Lucid GigE: "
                f"{len(self.cameras)}"
            )
        except Exception as e:
    ...
    def _update_camera_list(self):
        ...
        if not self.cameras:
            self.tree.insert(
                '', 'end',
                values=('Устройства Lucid Triton не обнаружены',
                        '—', '—', '—', '—')
            )
            self.status_bar.config(
                text="Устройства не найдены. Проверьте питание и Ethernet."
            )
        else:
            for cam in self.cameras:
                ...
                status_display = {
                    ...
                    'Failed': 'Ошибка интерфейса'
                }.get(status, 'Неизвестно')
                ip = cam.get('ip', 'N/A') or 'N/A'

                self.tree.insert(
                    '', 'end',
                    values=(cam.get('name', 'Unknown Triton'),
                            'GigE Vision',
                            cam.get('serial', 'N/A'),
                            ip,
                            status_display),
            ...
            self.status_bar.config(
                text=f"Обнаружено активных сетевых GigE камер: "
                     f"{len(self.cameras)}"
            )
    ...
    def on_select_camera(self, event):
        ...
        if selection:
            ...
            if selected_index < len(self.cameras):
                ...
                self.status_bar.config(
                    text=f"Выбрана: {self.selected_camera.get('name')} "
                         f"| SN: {self.selected_camera.get('serial')}"
                )
    ...
    def connect_camera(self):
        if not self.selected_camera:
            messagebox.showwarning(
                "Предупреждение",
                "Пожалуйста, сначала выберите устройство из списка."
            )
            return
        ...
        try:
            self.log_info(f"Подключение к {camera_name}...")
            self.connect_btn.config(state=tk.DISABLED)
            ...
            self.log_success(
                f"Прямой стрим-канал с устройством {camera_name} установлен."
            )
            ...
            self.video_info_label.config(
                text=f"Камера: {camera_name} (GigE Vision)"
            )
            ...
            error_msg = f"Ошибка Arena API при связи с камерой: {e}"
            self.log_error(error_msg)
    ...
    def start_video_stream(self):
        ...
        self.video_status.config(
            text="Статус: Активация матрицы, приём буферов..."
        )
    ...
    def update_ui_loop(self):
        ...
        self.root.after(60, self.update_ui_loop)
    ...
    def start_recording(self):
        if not self.current_camera:
            messagebox.showwarning(
                "Предупреждение", "Сначала подключитесь к камере."
            )
            return
        if self.is_recording:
            return
        ...
        try:
            ...
            self.log_success(
                f"Запущено сохранение сессии захвата: "
                f"{self.current_record_path}"
            )
            self.video_status.config(
                text="Статус: Запись идёт (отображение выкл.)"
            )

        except Exception as e:
            self.log_error(f"Не удалось инициализировать рекордер: {e}")
            messagebox.showerror(
    ...
    def stop_recording(self):
        ...
        try:
            ...
            if saved_path:
                self.log_success(f"Video-файл успешно сохранён: {saved_path}")
                self.video_status.config(
    ...
    def take_snapshot(self):
        ...
        try:
            ...
            self.log_success(
                f"Статический PNG-снимок матрицы сохранён: {filepath}"
            )
