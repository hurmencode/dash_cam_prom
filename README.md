...
**Dash Cam Prom** — это десктопное приложение для работы с камерами (промышленные GigE камеры). Приложение позволяет:
- 🔍 **Сканировать** доступные камеры
- 📷 **Подключаться** к камерам (GigE через Aravis)
...
### Основные функции
- Поиск камер — сканирует доступные GigE камеры
...
- Запись видео:
  ...
  - Сжатый (MJPG) — хорошее качество

- Снимки — сохраняет текущий кадр в PNG

- Логи — все действия записываются в лог

**При съемке видео отображение видео отключается**
